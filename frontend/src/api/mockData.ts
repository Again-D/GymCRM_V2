...
import { DEFAULT_TRAINER_PAYROLL_SESSION_UNIT_PRICE } from "../pages/settlements/modules/types";
...
const ADMIN_CREATABLE_ROLE_CODES = new Set([
	ROLE_MANAGER,
	ROLE_TRAINER,
	ROLE_DESK,
]);
const ADMIN_RESETTABLE_ROLE_CODES = new Set([
	ROLE_MANAGER,
	ROLE_TRAINER,
	ROLE_DESK,
]);
...
const PASSWORD_POLICY_MESSAGE =
	"8자 이상이며 문자, 숫자, 특수문자를 모두 포함해야 합니다.";
...
const initialTrainerAvailabilityByUserId = new Map<
	number,
	MockTrainerAvailabilityState
>([
	[
		41,
		{
...
let mockGxExceptions = initialMockGxExceptions.map((exception) => ({
	...exception,
}));
let mockSettlementTransactions = initialSettlementTransactions.map(
	(transaction) => ({ ...transaction }),
);
let mockTrainerPayrollAggregates = initialTrainerPayrollAggregates.map(
	(aggregate) => ({ ...aggregate }),
);
...
let lockerSlotIdSeed =
	Math.max(0, ...initialLockerSlots.map((slot) => slot.lockerSlotId)) + 1;
...
function cloneTrainerSettlementSnapshots(
	source: MockTrainerSettlementSnapshot[],
) {
...
function mutationEnvelope<T>(result: {
	ok: boolean;
	message: string;
	data?: T;
}): ApiEnvelope<T | null> {
...
function deriveAccessAlertSummary(): AccessAlertSummary {
	const deniedEvents = mockAccessEvents.filter(
		(event) => event.eventType === "ENTRY_DENIED",
	);
	const byReason = new Map<string, number>();
	const byMember = new Map<
		number,
		AccessAlertSummary["repeatedDeniedMembers"][number]
	>();
	...
	const deniedReasonCounts = Array.from(byReason.entries())
		...
		.sort(
			(left, right) =>
				right.deniedCount - left.deniedCount ||
				left.denyReason.localeCompare(right.denyReason),
		);

	const recentDeniedEvents = [...deniedEvents]
		.sort(
			(left, right) =>
				Date.parse(right.processedAt) - Date.parse(left.processedAt),
		)
		.slice(0, 20)
		.map((event) => ({
...
function filterLockerSlots(url: URL) {
	const lockerStatus =
		url.searchParams.get("lockerStatus")?.trim().toUpperCase() ?? "";
	const lockerZone =
		url.searchParams.get("lockerZone")?.trim().toUpperCase() ?? "";
...
function formatBucketLabel(
	...
	granularity: SettlementTrendGranularity,
) {
...
function filterSalesDashboard(url: URL): SalesDashboard {
	...
	const parsedExpiringWithinDays = Number.parseInt(
		url.searchParams.get("expiringWithinDays") ?? "7",
		10,
	);
	const expiringWithinDays = Number.isFinite(parsedExpiringWithinDays)
		? parsedExpiringWithinDays
		: 7;
	const monthPrefix = baseDate.slice(0, 7);

	const todayTransactions = mockSettlementTransactions.filter(
		(transaction) => transaction.transactionDate === baseDate,
	);
	const monthTransactions = mockSettlementTransactions.filter((transaction) =>
		transaction.transactionDate.startsWith(monthPrefix),
	);
	const newMemberCount = mockMembers.filter(
		(member) => member.joinDate === baseDate,
	).length;
	...
	return {
		baseDate,
		expiringWithinDays,
		todayNetSales: todayTransactions.reduce(
			(sum, transaction) =>
				sum + transaction.grossSales - transaction.refundAmount,
			0,
		),
		monthNetSales: monthTransactions.reduce(
			(sum, transaction) =>
				sum + transaction.grossSales - transaction.refundAmount,
			0,
		),
		newMemberCount,
		expiringMemberCount,
		refundCount: todayTransactions.filter(
			(transaction) => transaction.refundAmount > 0,
		).length,
	};
}
...
function filterSettlementReport(url: URL): SettlementReport {
	...
	for (const transaction of mockSettlementTransactions) {
		...
		const bucketStartDate = toBucketStartDate(
...
function filterSettlementRecentAdjustments(
	url: URL,
): SettlementRecentAdjustment[] {
	const startDate = url.searchParams.get("startDate")?.trim() ?? todayText();
	const endDate = url.searchParams.get("endDate")?.trim() ?? todayText();
	...
	return mockSettlementTransactions
		...
		.map((transaction) => {
			...
			const member = mockMembers.find(
				(item) => item.memberId === transaction.memberId,
			);
			return {
				transaction,
		...
		.filter(
			({ transaction, productName }) =>
				!productKeyword || productName.includes(productKeyword),
		)
...
function filterSettlementReceivables(url: URL): SettlementReceivables {
	...
	const rows = initialSettlementReceivables
		...
		.map((row) => ({
			...
			reminderChannel: (row.followUpDate <= baseDate ? "CRM" : "REVIEW") as
				| "CRM"
				| "REVIEW",
		}));
	...
	return {
		baseDate,
		limit,
		totalOutstandingAmount: rows.reduce(
...
function buildTrainerPayrollReport(
	settlementMonth: string,
	sessionUnitPrice: number,
): TrainerPayrollReport {
	...
	if (confirmedRows.length > 0) {
		...
		return {
			...
			totalCompletedClassCount: rows.reduce(
				(sum, row) => sum + row.completedClassCount,
				0,
			),
...
function filterTrainerPayroll(url: URL): TrainerPayrollReport {
	const settlementMonth =
		url.searchParams.get("settlementMonth")?.trim() ?? todayText().slice(0, 7);
	const sessionUnitPrice = Number(
		url.searchParams.get("sessionUnitPrice") ?? "50000",
	);
	const scopedTrainerUserId = parseOptionalTrainerUserId(
		url.searchParams.get("trainerUserId"),
	);
...
function filterTrainerMonthlyPtSummary(url: URL): TrainerMonthlyPtSummary {
	const settlementMonth =
		url.searchParams.get("settlementMonth")?.trim() ?? todayText().slice(0, 7);
	const trainerUserId = parseOptionalTrainerUserId(
		url.searchParams.get("trainerUserId"),
	);
	const report = scopeTrainerPayrollReport(
		buildTrainerPayrollReport(
			settlementMonth,
	...
	const fallbackTrainer = mockTrainers.find(
		(trainer) => trainer.userId === trainerUserId,
	);
...
function scopeTrainerPayrollReport(
	report: TrainerPayrollReport,
	trainerUserId: number | null,
) {
...
function buildTrainerSettlementPreview(
	query: TrainerSettlementPreviewQuery,
): TrainerSettlementPreviewReport {
	...
	const rows: TrainerSettlementPreviewRow[] = baseRows.map((row) => {
		...
		const hasRateWarning =
			rates.ptRatePerSession == null || rates.gxRatePerSession == null;
		const ptAmount =
			rates.ptRatePerSession == null
				? null
				: ptSessions * rates.ptRatePerSession;
		const gxAmount =
			rates.gxRatePerSession == null
				? null
				: gxSessions * rates.gxRatePerSession;
		const totalAmount =
	...
	const hasConflict = mockPeriodSettlements.some(
		(settlement) =>
	...
	return {
		...
		summary: {
			...
			cancelledSessions: rows.reduce(
				(sum, row) => sum + row.cancelledSessions,
				0,
			),
	...
	return deriveMembers().filter((member) => {
		...
		const matchesMembershipFilters =
			...
			visibleMemberships.some((membership) => {
				...
				return (
					matchesTrainer && matchesProduct && matchesDateFrom && matchesDateTo
				);
			});
		return (
			matchesName &&
...
function filterMockUserAccounts(
	...
	const filtered = mockUserAccounts
		...
		.filter(
			(user) => !q || user.loginId.includes(q) || user.userName.includes(q),
		)
...
function createMockUserAccountFromRequest(
	...
	const temporaryPassword = requireMockText(
...
function resetMockUserPasswordFromRequest(
	...
	const targetUser = getMockUserAccountInCurrentCenter(
		targetUserId,
		authSession,
	);
	if (!targetUser) {
		throwMockApiError(
			404,
			"NOT_FOUND",
			`사용자를 찾을 수 없습니다. userId=${targetUserId}`,
		);
	}

	ensureMockResetAllowed(
		actor.primaryRole,
		targetUser.roleCode,
		actor.userId,
		targetUser.userId,
...
function changeMockPasswordFromRequest(
	...
	const payload = (body ?? {}) as Record<string, unknown>;
	const currentPassword = normalizeMockOptionalPassword(
...
export function updateMockUserStatus(
	userId: number,
	userStatus: MockUserAccountRecord["userStatus"],
) {
	const updatedAt = new Date().toISOString();
	let nextUser: MockUserAccountRecord | null = null;
	mockUserAccounts = mockUserAccounts.map((user) => {
...
export function resetMockData() {
	...
	mockTrainerPayrollAggregates = initialTrainerPayrollAggregates.map(
		(aggregate) => ({ ...aggregate }),
	);
...
export function createMockMembership(input: {
	...
	const membership: PurchasedMembership = {
		...
		productCategorySnapshot: input.productNameSnapshot.includes("PT")
			? "PT"
...
function getMockPtReservationCandidates(
	...
	const rule = availability?.weeklyRules.find(
		(item) => item.dayOfWeek === normalizedWeekday,
	);
	if (!rule) {
		return {
			date,
	...
	const trainerReservations = Array.from(mockReservationsByMemberId.values())
		.flat()
		.map((reservation) => {
			const schedule = mockReservationSchedules.find(
				(item) => item.scheduleId === reservation.scheduleId,
			);
			return schedule &&
				schedule.trainerUserId === trainerUserId &&
	...
	const memberBlocks = memberReservations
		.map((reservation) =>
			mockReservationSchedules.find(
				(item) => item.scheduleId === reservation.scheduleId,
			),
		)
		.filter(
			(item): item is ReservationScheduleSummary =>
				item != null &&
				item.scheduleType !== undefined &&
				memberReservations.some(
					(reservation) =>
						reservation.scheduleId === item.scheduleId &&
						reservation.reservationStatus === "CONFIRMED",
				),
		);
	...
	for (
		...
		startMinutes + 60 <= windowEndMinutes;
		startMinutes += 30
	) {
		const candidateStartHour = String(Math.floor(startMinutes / 60)).padStart(
			2,
			"0",
		);
		const candidateStartMinute = String(startMinutes % 60).padStart(2, "0");
		const candidateEndTotalMinutes = startMinutes + 60;
		const candidateEndHour = String(
			Math.floor(candidateEndTotalMinutes / 60),
		).padStart(2, "0");
		const candidateEndMinute = String(candidateEndTotalMinutes % 60).padStart(
			2,
			"0",
		);
		const candidateStartAt = formatMockIso(
			date,
			`${candidateStartHour}:${candidateStartMinute}:00`,
		);
		const candidateEndAt = formatMockIso(
			date,
			`${candidateEndHour}:${candidateEndMinute}:00`,
		);
		...
		const overlapsTrainer = trainerReservations.some((schedule) => {
			...
			return (
				candidateStartTime < scheduleEnd && scheduleStart < candidateEndTime
			);
		});
...
export function createMockPtReservation(input: {
	...
	const trainer = mockTrainers.find(
		(item) => item.userId === input.trainerUserId,
	);
	...
	mockMemberMemberships.set(
		...
		memberships.map((membership) =>
			...
			membership.remainingCount != null
				? {
						...membership,
						remainingCount: Math.max(0, membership.remainingCount - 1),
					}
				: membership,
		),
	);
...
export function createMockLockerSlots(input: MockLockerCreateInput[]) {
	...
	for (const [index, row] of input.entries()) {
		...
		if (!lockerZone) {
			return {
				ok: false as const,
				message: `${index + 1}행: 구역을 입력해야 합니다.`,
			};
		}
		if (!Number.isInteger(row.lockerNumber) || row.lockerNumber < 1) {
			return {
				ok: false as const,
				message: `${index + 1}행: 번호를 입력해야 합니다.`,
			};
		}
		...
		if (seenCodes.has(lockerCode)) {
			return {
				ok: false as const,
				message: `중복 라커 코드가 있습니다: ${lockerCode}`,
			};
		}
		if (existingCodes.has(lockerCode)) {
			return {
				ok: false as const,
				message: `이미 존재하는 라커 코드입니다: ${lockerCode}`,
			};
		}
...
function buildMockTrainerSummary(trainer: MockTrainerRecord): TrainerSummary {
	...
	const todayConfirmedReservationCount = assignedMemberIds.reduce(
		(count, memberId) => {
			const reservations = mockReservationsByMemberId.get(memberId) ?? [];
			return (
				count +
...
function syncMockTrainerAccount(user: MockUserAccountRecord) {
	if (user.roleCode === "ROLE_TRAINER") {
		const existingTrainer = mockTrainers.find(
			(trainer) => trainer.userId === user.userId,
		);
		if (existingTrainer) {
	...
	const existingTrainer = mockTrainers.some(
		(trainer) => trainer.userId === user.userId,
	);
	if (existingTrainer) {
		mockTrainers = mockTrainers.filter(
			(trainer) => trainer.userId !== user.userId,
		);
...
function currentMockCenterId(
	authSession: MockAuthSessionContext | null | undefined,
) {
	return authSession?.centerId ?? 1;
}

function currentMockPasswordForSession(
	authSession: MockAuthSessionContext | null | undefined,
) {
	if (!authSession) {
		return null;
	...
	if (
		...
		authSession.currentPassword.trim()
	) {
		return authSession.currentPassword.trim();
	}
	return (
		mockUserPasswords.get(authSession.userId) ??
		defaultMockPasswordForRole(authSession.primaryRole)
	);
}
...
function buildMockLifecycleIdentityFromUser(
	user: MockUserAccountRecord,
...
function ensureMockCreateRoleAllowed(
	actorRole: string,
...
function ensureMockResetAllowed(
	actorRole: string,
	...
	if (actorUserId === targetUserId) {
		throwMockApiError(
			403,
			"ACCESS_DENIED",
			"자기 계정은 /my-account에서만 변경할 수 있습니다.",
		);
	}
	...
	if (actorRole === ROLE_ADMIN) {
		if (!ADMIN_RESETTABLE_ROLE_CODES.has(targetRoleCode)) {
			throwMockApiError(
				403,
				"ACCESS_DENIED",
				"관리자 역할은 지정된 운영 역할만 초기화할 수 있습니다.",
			);
		}
		return;
	}
...
export function createMockUserAccount(input: {
	...
	const duplicate = mockUserAccounts.find(
		(user) =>
			user.centerId === input.centerId && user.loginId === input.loginId,
	);
	if (duplicate) {
		throwMockApiError(409, "CONFLICT", "이미 존재하는 loginId입니다.");
	}
	...
	const nextUserId =
		Math.max(0, ...mockUserAccounts.map((user) => user.userId)) + 1;
...
function buildMockTrainerAvailabilitySnapshot(
	...
	const ruleByDay = new Map(
		state.weeklyRules.map((rule) => [rule.dayOfWeek, rule]),
	);
...
export function getMockTrainerAvailabilitySnapshot(
	userId: number,
	month: string,
) {
...
function buildMockGxScheduleSnapshot(
	...
	const generatedSchedules = Array.from({ length: lastDay }, (_, index) => {
		...
		return visibleRules.flatMap((rule) => {
			...
			const trainerUserId =
				exception?.overrideTrainerUserId ?? rule.trainerUserId;
			const trainer = mockTrainers.find(
				(item) => item.userId === trainerUserId,
			);
...
export function getMockResponse(
	...
	if (
		url.pathname.match(/^\/api\/v1\/auth\/users\/\d+\/password-reset$/) &&
		method === "POST"
	) {
	...
	if (url.pathname === "/api/v1/reservations") {
		...
		return envelope(
			reservations
				...
				.filter(
					(reservation) => !status || reservation.reservationStatus === status,
				)
	...
	if (url.pathname === "/api/v1/reservations/schedules") {
		...
		const visibleSchedules = mockReservationSchedules.filter(
			(schedule) =>
				isMockScheduleVisibleByDefault(schedule) ||
				requestedScheduleIds.includes(schedule.scheduleId),
		);
		return envelope(visibleSchedules.map((schedule) => ({ ...schedule })));
	}
	...
	if (url.pathname === "/api/v1/lockers" && method === "POST") {
		return mutationEnvelope(
			createMockLockerSlot(body as MockLockerCreateInput),
		);
	}
	...
	if (url.pathname === "/api/v1/lockers/batch" && method === "POST") {
		return mutationEnvelope(
			createMockLockerSlots(
				(body as { items?: MockLockerCreateInput[] })?.items ?? [],
			),
		);
	}
	...
	if (
		url.pathname === "/api/v1/crm/messages/triggers/inactive-member-campaign"
	) {
		const inactiveDays = Number(
			(body as { inactiveDays?: number })?.inactiveDays ?? 30,
		);
		return mutationEnvelope(
			triggerMockCrmInactiveMemberCampaign(
				Number.isFinite(inactiveDays) ? inactiveDays : 30,
			),
		);
	}
	...
	if (url.pathname === "/api/v1/settlements/preview") {
		return envelope(
			buildTrainerSettlementPreview({
				trainerId: url.searchParams.get("trainerId")?.trim() || "ALL",
				settlementMonth:
					url.searchParams.get("settlementMonth")?.trim() ??
					todayText().slice(0, 7),
			}),
		);
	}
...
export async function createMockTrainerSettlementConfirm(input: {
	...
	const report = buildTrainerPayrollReport(
		input.settlementMonth,
		input.sessionUnitPrice,
	);
	...
	mockTrainerSettlementSnapshots = [
		...mockTrainerSettlementSnapshots.filter(
			(snapshot) => snapshot.settlementMonth !== input.settlementMonth,
		),
		...report.rows.map((row) => ({
...
function toMockWorkspace(
	settlement: MockPeriodSettlementRecord,
): TrainerSettlementWorkspace {
	...
	const totalSessions = settlement.rows.reduce(
		(sum, row) => sum + row.totalSessions,
		0,
	);
	const completedSessions = settlement.rows.reduce(
		(sum, row) => sum + row.completedSessions,
		0,
	);
	const cancelledSessions = settlement.rows.reduce(
		(sum, row) => sum + row.cancelledSessions,
		0,
	);
	const noShowSessions = settlement.rows.reduce(
		(sum, row) => sum + row.noShowSessions,
		0,
	);
	const ptSessions = settlement.rows.reduce(
		(sum, row) => sum + row.ptSessions,
		0,
	);
	const gxSessions = settlement.rows.reduce(
		(sum, row) => sum + row.gxSessions,
		0,
	);
	const ptAmount = settlement.rows.reduce(
		(sum, row) => sum + (row.ptAmount ?? 0),
		0,
	);
	const gxAmount = settlement.rows.reduce(
		(sum, row) => sum + (row.gxAmount ?? 0),
		0,
	);
...
export async function createMockTrainerSettlementCreate(
	input: TrainerSettlementPreviewQuery,
) {
	...
	const existing = mockPeriodSettlements.find(
		(settlement) =>
...
export async function confirmMockTrainerSettlement(settlementId: number) {
	const target = mockPeriodSettlements.find(
...
export async function downloadMockCanonicalTrainerSettlementDocument(
	settlementId: number,
	trainerId: string,
) {
	const target = mockPeriodSettlements.find(
	...
	const row = target.rows.find(
		(item) => String(item.trainerUserId) === trainerId,
	);
	if (!row) {
		return {
			ok: false,
...
export async function downloadMockSalesSettlementReport(filters: {
	...
	const report = filterSettlementReport(
		new URL(
			`/api/v1/settlements/sales-report?${query.toString()}`,
			"http://local.mock",
		),
	);
