...
export function useLockerQueries() {
  ...
  const [lockerAssignments, setLockerAssignments] = useState<
    LockerAssignment[]
  >([]);
  const [lockerAssignmentsLoading, setLockerAssignmentsLoading] =
    useState(false);
  ...
  const lockerAssignmentsCacheRef = useRef(
    new Map<string, LockerAssignment[]>(),
  );
  const lockerSlotsInflightRef = useRef(
    new Map<string, Promise<LockerSlot[]>>(),
  );
  const lockerAssignmentsInflightRef = useRef(
    new Map<string, Promise<LockerAssignment[]>>(),
  );
  const lockerSlotsVersion = useQueryInvalidationVersion("lockerSlots");
  const lockerAssignmentsVersion =
    useQueryInvalidationVersion("lockerAssignments");

  const loadLockerSlots = useCallback(
    async (filters?: LockerFilters) => {
      ...
      try {
        ...
        if (!responsePromise) {
          responsePromise = apiGet<LockerSlot[]>(
  ...
  const loadLockerAssignments = useCallback(
      ...
      try {
        ...
        if (lockerAssignmentsCacheRef.current.has(cacheKey)) {
          ...
          setLockerAssignments(
            lockerAssignmentsCacheRef.current.get(cacheKey) ?? [],
          );
          return;
        }

        let responsePromise =
          lockerAssignmentsInflightRef.current.get(cacheKey);
        if (!responsePromise) {
          responsePromise = apiGet<LockerAssignment[]>(
            `/api/v1/lockers/assignments?${query}`,
          )
            .then((response) => response.data)
            .finally(() =>
              lockerAssignmentsInflightRef.current.delete(cacheKey),
            );
  ...
  const reloadLockerData = useCallback(
    async (filters?: LockerFilters, activeOnly = false) => {
      await Promise.all([
        loadLockerSlots(filters),
        loadLockerAssignments(activeOnly),
      ]);
    },
