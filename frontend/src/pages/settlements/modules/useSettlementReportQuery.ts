...
export function useSettlementReportQuery({
  getDefaultFilters,
}: UseSettlementReportQueryOptions) {
  const [settlementReport, setSettlementReport] =
    useState<SettlementReport | null>(null);
  const [settlementReportLoading, setSettlementReportLoading] = useState(false);
  const [settlementReportError, setSettlementReportError] = useState<
    string | null
  >(null);
  const [settlementReportMessage, setSettlementReportMessage] = useState<
    string | null
  >(null);
  const requestIdRef = useRef(0);
  const cacheRef = useRef(new Map<string, SettlementReportCacheValue>());
  const inflightRef = useRef(
    new Map<string, Promise<SettlementReportCacheValue>>(),
  );
  const getDefaultFiltersRef = useLatestRef(getDefaultFilters);
  const settlementReportVersion =
    useQueryInvalidationVersion("settlementReport");
  const productsVersion = useQueryInvalidationVersion("products");

  const loadSettlementReport = useCallback(
    async (filters?: SettlementReportFilters) => {
      ...
      try {
        ...
        if (!responsePromise) {
          responsePromise = apiGet<SettlementReport>(
            `/api/v1/settlements/sales-report?${query}`,
          )
            .then((response) => ({
              data: response.data,
