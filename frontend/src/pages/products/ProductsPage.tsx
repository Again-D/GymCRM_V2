...
export default function ProductsPage() {
	...
	const productColumns: ColumnsType<ProductRecord> = [
		...
		},
		{
			...
			key: "category",
			render: (category) => (
				<Tag color="blue">{productCategoryLabel(category)}</Tag>
			),
		},
		{
			title: "가격",
	...
	const averagePrice =
		products.length > 0
			? formatCurrency(
					Math.floor(
						products.reduce((acc, product) => acc + product.priceAmount, 0) /
							products.length,
					),
				)
			: "0";
		...
		<Flex vertical gap={24}>
				...
				<Flex justify="space-between" align="start" wrap="wrap" gap={16}>
					<Space direction="vertical" size={4}>
						...
						<Paragraph type="secondary" style={{ margin: 0, maxWidth: 640 }}>
							회원권, PT, 그룹수업 상품을 등록하고 판매 정책을 개별 설정할 수
							있습니다.
						</Paragraph>
			...
			<Row gutter={[16, 16]}>
				{[
					...
					{
						label: "시스템 권한",
						value: canMutateProducts ? "풀 액세스" : "조회 전용",
						hint: canMutateProducts ? "수정 가능" : "작업 제한",
					},
				].map((stat) => (
				...
				<Flex vertical gap={16}>
									...
									<Select
										...
										onChange={(val) =>
											setProductFilters((prev) => ({
												...prev,
												category: val as ProductFilters["category"],
											}))
										}
					...
					{(productPanelError || productsQueryError) && (
						<Alert
							type="error"
							showIcon
							message={productPanelError ?? productsQueryError}
							closable
						/>
					)}
			...
			<Modal
				...
				title={
					productFormMode === "create"
						? "신규 상품 등록"
						: `상품 #${selectedProductId} 수정 및 상세`
				}
				footer={[
					<Button key="cancel" onClick={closeProductForm}>
						취소
					</Button>,
					<Button
						key="submit"
					...
					<Form layout="vertical">
								...
								<Form.Item label="상품 유형" required>
									<Select
										...
										onChange={(val) =>
											setProductForm((prev) => ({
												...prev,
												productType: val as ProductFormState["productType"],
											}))
										}
										options={[
						...
						<Row gutter={16}>
							<Col span={12}>
								<Form.Item
									label="유효 기간(일)"
									required={productForm.productType === "DURATION"}
								>
									<Input
										type="number"
							...
							<Col span={12}>
								<Form.Item
									label="총 이용 횟수"
									required={productForm.productType === "COUNT"}
								>
									<Input
										type="number"
						...
						<Card
							size="small"
							title={<Text strong>운영 및 정기 정책</Text>}
								...
								<Space size="large">
									...
									<Checkbox
										...
										onChange={(e) =>
											setProductForm((prev) => ({
												...prev,
												allowTransfer: e.target.checked,
											}))
										}
									>
										타인 양도 허용
											...
											<Form.Item style={{ marginTop: 12, marginBottom: 0 }}>
												<Checkbox
													...
													onChange={(e) =>
														setProductForm((prev) => ({
															...prev,
															allowHoldBypass: e.target.checked,
														}))
													}
												>
													관리자 제한 우회 홀딩 허용
