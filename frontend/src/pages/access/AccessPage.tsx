  ...
  const memberColumns: ColumnsType<MemberSummary> = [
    {
      ...
      render: (_, record) => (
        <Space direction="vertical" size={2}>
          ...
          <Text type="secondary" style={{ fontSize: '0.75rem' }}>ID: {record.memberId} · {record.phone}</Text>
        </Space>
      )
    },
    {
      ...
      key: "status",
      render: (status) => (
        <Tag color={status === '정상' ? 'success' : status === '홀딩중' ? 'processing' : 'default'}>
          {status}
        </Tag>
      )
    },
      ...
      <Card>
        <Flex justify="space-between" align="start" wrap="wrap" gap={16}>
          <Space direction="vertical" size={4}>
            <Text type="secondary" style={{ fontSize: '0.72rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#1677ff' }}>출입 제어</Text>
            <Title level={2} style={{ margin: 0 }}>출입 모니터링</Title>
            <Paragraph type="secondary" style={{ margin: 0, maxWidth: 640 }}>
              회원 상태를 확인하고 현재 입장 현황을 추적하며 리셉션 출입 처리를 한 화면에서 수행합니다.
            </Paragraph>
        ...
        <Row gutter={[16, 16]}>
          {[
            { label: "현재 입장", value: accessPresence?.openSessionCount ?? 0, hint: "현재 센터 내부에 있는 총 회원 수" },
            { label: "오늘 입장", value: accessPresence?.todayEntryGrantedCount ?? 0, hint: "금일 승인된 입장 총계" },
            { label: "오늘 퇴장", value: accessPresence?.todayExitCount ?? 0, hint: "금일 기록된 퇴장 총계" },
            { label: "입장 거부", value: accessPresence?.todayEntryDeniedCount ?? 0, hint: "승인되지 않은 무효한 시도" }
          ].map((kpi) => (
            <Col xs={12} sm={6} key={kpi.label}>
              <Statistic 
                title={<Text type="secondary" style={{ fontSize: '0.73rem', fontWeight: 700, textTransform: 'uppercase' }}>{kpi.label}</Text>}
                value={kpi.value} 
                valueStyle={{ fontWeight: 800 }}
                suffix={<Text type="secondary" style={{ fontSize: '0.75rem', display: 'block' }}>{kpi.hint}</Text>}
              />
            </Col>
          ))}
        </Row>
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={16}>
          <Card 
            title={
              <Space direction="vertical" size={2}>
                <Title level={5} style={{ margin: 0 }}>회원 디렉터리</Title>
                <Text type="secondary" style={{ fontSize: '0.84rem' }}>출입 처리를 위한 회원을 검색하고 선택합니다.</Text>
              </Space>
            }
          >
            <Flex vertical gap={16}>
              ...
              <Table<MemberSummary>
                ...
                pagination={false}
                locale={{
                  emptyText: !isLiveAccessRoleSupported ? "제한된 권한 상태입니다." : "조회할 회원을 검색해 주세요."
                }}
              />
        ...
        <Col xs={24} lg={8}>
          <Flex vertical gap={24}>
            <Card 
              title={
                <Space direction="vertical" size={2}>
                  <Title level={5} style={{ margin: 0 }}>현재 입장 회원</Title>
                  <Text type="secondary" style={{ fontSize: '0.84rem' }}>지금 센터 내부에 있는 회원 목록입니다.</Text>
                </Space>
              }
            >
              <Table
                ...
                columns={[
                  { title: "회원", dataIndex: "memberName", key: "name", render: (name) => <Text strong>{name}</Text> },
                  { title: "입장 시각", dataIndex: "entryAt", key: "time", align: "right", render: (time) => <Text type="secondary" style={{ fontSize: '0.75rem' }}>{formatDateTime(time)}</Text> }
                ]}
            ...
            <Card 
              title={
                <Space direction="vertical" size={2}>
                  <Title level={5} style={{ margin: 0 }}>출입 이력</Title>
                  <Text type="secondary" style={{ fontSize: '0.84rem' }}>선택한 회원 또는 최근 출입 이벤트를 확인합니다.</Text>
                </Space>
              }
            >
              <Table
                  ...
                  { 
                    ...
                    render: (_, record) => (
                      <Space direction="vertical" size={0}>
                        <Text strong style={{ fontSize: '0.875rem' }}>ID: #{record.memberId}</Text>
                        <Text type="secondary" style={{ fontSize: '0.75rem' }}>{formatDateTime(record.processedAt)}</Text>
                      </Space>
                    )
                  },
                  { 
                    ...
                    align: "right",
                    render: (type) => {
                      const label = eventLabel[type] || { label: type, color: 'default' };
        ...
        <Flex vertical gap={24}>
          <Card size="small" style={{ background: '#f5f5f5' }}>
            <Flex justify="space-between" align="center">
              <Space direction="vertical" size={2}>
                <Text type="secondary" style={{ fontSize: '0.72rem', textTransform: 'uppercase', fontWeight: 700 }}>현재 컨텍스트 처리</Text>
                <Title level={5} style={{ margin: 0 }}>
                  {selectedMember ? `${selectedMember.memberName} (#${selectedMember.memberId})` : "선택된 회원 없음"}
                </Title>
              </Space>
              <Space>
                <Button
          ...
          {(accessPanelMessage || accessPanelError) && (
            <Space direction="vertical" style={{ width: '100%' }}>
              {accessPanelMessage && <Alert type="success" message={accessPanelMessage} showIcon />}
              {accessPanelError && <Alert type="error" message={accessPanelError} showIcon />}
            </Space>
          )}

          <section>
            <Divider orientation="left" style={{ margin: '0 0 16px' }}>회원 개별 출입 기록</Divider>
            <Table
              size="small"
              rowKey="accessEventId"
              dataSource={accessEventsPagination.pagedItems}
              pagination={false}
              columns={[
                { title: "이벤트 시각", dataIndex: "processedAt", key: "time", render: (time) => formatDateTime(time) },
                { 
                  title: "유형", 
                  dataIndex: "eventType", 
                  key: "type", 
