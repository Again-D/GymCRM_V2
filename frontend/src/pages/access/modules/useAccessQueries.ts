...
export function useAccessQueries(memberId?: number | null) {
  ...
  const alertsQuery = useQuery({
    queryKey: queryKeys.access.list({ scope: "alerts" }),
    queryFn: async () => {
      const response = await apiGet<AccessAlertSummary>("/api/v1/access/alerts");
      return response.data;
    },
    staleTime: queryPolicies.list.staleTime,
    gcTime: queryPolicies.list.gcTime,
  ...
  const { refetch: refetchAlerts } = alertsQuery;
  ...
  return useMemo(
    () => ({
      ...
      accessAlerts: alertsQuery.data ?? null,
      ...
      accessAlertsLoading: alertsQuery.isFetching || alertsQuery.isPending,
      accessAlertsError: alertsQuery.error
        ? toUserFacingErrorMessage(alertsQuery.error, "비정상 출입 요약을 불러오지 못했습니다.")
        : null,
      accessQueryError:
      ...
      refetchAccessAlerts: refetchAlerts,
    }),
    [
      eventsQuery.data,
