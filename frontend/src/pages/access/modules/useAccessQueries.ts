...
export function useAccessQueries() {
  ...
  const accessPresenceCacheRef = useRef(new Map<string, AccessPresenceSummary>());
  const accessEventsInflightRef = useRef(new Map<string, Promise<AccessEventRow[]>>());
  const accessPresenceInflightRef = useRef(new Map<string, Promise<AccessPresenceSummary>>());
  ...
  const loadAccessEvents = useCallback(async (memberId?: number | null) => {
    ...
    try {
      ...
      if (!responsePromise) {
        responsePromise = apiGet<AccessEventRow[]>(`/api/v1/access/events?${query}`)
      ...
      if (accessEventsRequestIdRef.current !== requestId) return;
      setAccessEvents([]);
      setAccessQueryError(error instanceof Error ? error.message : "출입 이벤트를 불러오지 못했습니다.");
    } finally {
  ...
  const loadAccessPresence = useCallback(async () => {
    ...
    try {
      ...
      if (!responsePromise) {
        responsePromise = apiGet<AccessPresenceSummary>("/api/v1/access/presence")
          .then((response) => response.data)
      ...
      setAccessQueryError(error instanceof Error ? error.message : "현재 입장 현황을 불러오지 못했습니다.");
    } finally {
