...
export function useReservationTargetsQuery() {
  ...
  const [reservationTargets, setReservationTargets] = useState<
    ReservationTargetSummary[]
  >([]);
  const [reservationTargetsKeyword, setReservationTargetsKeyword] =
    useState("");
  const [reservationTargetsLoading, setReservationTargetsLoading] =
    useState(false);
  const [reservationTargetsError, setReservationTargetsError] = useState<
    string | null
  >(null);
  const requestIdRef = useRef(0);
  const cacheRef = useRef(new Map<string, ReservationTargetSummary[]>());
  const inflightRef = useRef(
    new Map<string, Promise<ReservationTargetSummary[]>>(),
  );
  const authUserRef = useLatestRef(authUser);
  const reservationTargetsKeywordRef = useLatestRef(reservationTargetsKeyword);
  const reservationTargetsVersion =
    useQueryInvalidationVersion("reservationTargets");

  const loadReservationTargets = useCallback(
    async (keyword?: string) => {
      ...
      try {
        ...
        if (cacheRef.current.has(cacheKey)) {
          ...
          setReservationTargets((prev) =>
            prev.length === 0 ? prev : (cacheRef.current.get(cacheKey) ?? []),
          );
          return;
        }
        ...
        if (!responsePromise) {
          responsePromise = apiGet<ReservationTargetSummary[]>(
            ...
            .then((response) =>
              filterMemberIdsForAuth(response.data, currentAuthUser),
            )
            .finally(() => {
        ...
        setReservationTargets((prev) =>
          scopedTargets.length === 0 && prev.length === 0
            ? prev
            : scopedTargets,
        );
      } catch (error) {
        ...
        setReservationTargetsError(
          error instanceof Error
            ? error.message
            : "예약 대상 회원을 불러오지 못했습니다.",
        );
      } finally {
        if (requestIdRef.current === requestId) {
