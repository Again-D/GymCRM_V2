...
describe("MemberQrPage", () => {
  ...
  it("clears the visible QR when refresh fails", async () => {
    vi.unstubAllGlobals();
    vi.stubGlobal("fetch", createFetchMock({ failOnRequest: 2, ttlSeconds: 30 }));
    ...
    expect(screen.getByText("오류를 확인한 뒤 새로고침을 눌러주세요.")).toBeTruthy();
  });
});

function createFetchMock(options?: { failOnRequest?: number; ttlSeconds?: number }) {
  let requestCount = 0;
  ...
  return vi.fn(async () => {
    ...
    const expiresAt = new Date(Date.parse(issuedAt) + ttlSeconds * 1000 * requestCount).toISOString();
