...
describe("useMemberManagementState", () => {
	...
	it("refreshes the selected member after edit success", async () => {
		...
		const originalDetail = getMockResponse("/api/v1/members/101")
		...
		const updatedDetail = getMockResponse("/api/v1/members/101")
			?.data as MemberDetail;
	...
	it("blocks member mutations for trainer role", async () => {
		...
		expect(result.current.memberFormError).toBe(
			"회원 정보를 변경할 권한이 없습니다.",
		);
