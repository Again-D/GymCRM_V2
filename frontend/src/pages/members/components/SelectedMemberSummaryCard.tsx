...
export function SelectedMemberSummaryCard({
	surface = "panel",
	action,
}: SelectedMemberSummaryCardProps) {
	const { selectedMember, selectedMemberError, selectedMemberLoading } =
	...
	const memberStatusLabel =
		selectedMember?.memberStatus === "ACTIVE" ? "활성" : "비활성";
	...
	async function openMemberQrLink(memberId: number) {
		...
		setQrLinkError(null);
		try {
			const response = await apiPost<{ memberQrPath: string }>(
				"/api/v1/access/qr/member-link",
				{
					memberId,
				},
			);
							...
							},
							{
								...
								children: (
									<Space direction="vertical" size={4}>
										<Typography.Text type="secondary">
											상세 조회에서는 링크를 노출하지 않습니다. 필요할 때만 QR
											링크를 발급합니다.
										</Typography.Text>
										<Button
											size="small"
											onClick={() =>
												void openMemberQrLink(selectedMember.memberId)
											}
