...
describe("MemberListSection", () => {
	...
	it("opens the selected member modal after a successful row click", async () => {
		...
		expect(
			screen.getByRole("dialog", { name: /김민수 회원 정보/ }),
		).toBeTruthy();
	...
	it("resets filters on reset button click", () => {
		...
		fireEvent.change(screen.getByPlaceholderText("이름 검색"), {
			target: { value: "김민수" },
		});
		fireEvent.change(screen.getByPlaceholderText("010-..."), {
		...
		expect(
			(screen.getByPlaceholderText("이름 검색") as HTMLInputElement).value,
		).toBe("");
		expect(
