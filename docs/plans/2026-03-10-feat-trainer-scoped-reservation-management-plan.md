...
- [x] 과거 일정 / 종료 전 노쇼 / 완료·취소 후 재조정 차단 테스트 추가
- [x] 프런트 액션 버튼 비활성화 및 안내 문구 정렬
- [x] prototype/jwt 모드 스모크 검증
...
- member summary와 reservation target list는 모두 `member` 단위 한 행으로 집계한다. 대표 membership 정보는 `end_date NULLS LAST, membership_id ASC` 기준으로 고르고, 최종 리스트 정렬은 `member_id DESC`를 사용한다.
...
## Acceptance Criteria
- [x] `ROLE_TRAINER` 계정으로 로그인 시 프런트 세션이 정상 동작한다.
- [x] membership에 담당 트레이너를 선택적으로 저장할 수 있다.
- [x] 회원관리 상단에 `트레이너별`, `상품별`, `기간별` 필터가 추가된다.
- [x] `기간별` 필터는 회원권 유효기간과 선택 구간이 겹치는 회원권 기준으로 동작한다.
- [x] `1주일`, `1개월`, `3개월`, `6개월` 빠른 기간 프리셋으로도 동일한 기간 조회가 가능하다.
- [x] 관리자 계정은 전체 예약권 보유 회원 리스트를 본다.
- [x] 트레이너 계정은 본인 담당 회원권을 가진 예약권 보유 회원만 본다.
- [x] trainer가 사용하는 member list/detail read 경로가 문서에 정의돼 있고, backend scope 강제가 포함된다.
- [x] 예약 관리에서 회원명 클릭 시 회원정보, 상품정보, 예약상태, 예약일정을 포함한 상세 모달을 볼 수 있다.
- [x] trainer의 상세 모달/상세 패널은 비담당 membership/예약을 노출하지 않는다.
- [x] 트레이너는 본인 담당 일정에 대해서만 취소/노쇼/허용된 상태 조정을 수행할 수 있다.
- [x] 과거 일정, 종료 전 노쇼, 완료/취소/노쇼 후 재조정은 프런트와 백엔드에서 모두 차단된다.
- [x] 교차센터 및 타 트레이너 데이터 접근이 차단된다.
- [x] `frontend` 빌드와 `backend` 관련 통합 테스트가 통과한다.
- [x] prototype/jwt 모드 검증 로그가 남는다.
