...
- [ ] 예약권 보유 회원 리스트용 reservation-target query 설계
...
- [ ] 예약 관리 탭을 `예약권 보유 회원 리스트 + 상세 패널` 구조로 재구성
- [ ] 역할별 데이터 범위 연결
- [ ] 회원명 클릭 상세 모달 추가
- [ ] 회원정보/상품정보/예약상태/예약일정 표시
- [ ] trainer detail payload가 비담당 membership/예약을 포함하지 않도록 DTO/query scope 고정
- [ ] 기존 direct-entry member picker 흐름과 충돌 없이 흡수
...
- [ ] 트레이너의 본인 담당 일정 조정만 허용하도록 서비스 검증 추가
- [ ] 교차센터/타 트레이너 대상 접근 차단 테스트 추가
