...
This plan has been completed in the isolated rebuild worktree branch:
...
The rebuild stage itself is no longer the active planning surface. Remaining follow-up work is now tracked separately in:
...
- [x] go 판단 이후 기존 member-context 흐름과 trainer reservation 제한이 새 구조에서 parity 수준으로 재현된다.
...
- [x] debounce/cache/dedupe가 필요한 검색 surface에서 request churn regression 없음
- [x] stale-response/reset invalidation regression tests 복제 또는 동등 coverage 확보
