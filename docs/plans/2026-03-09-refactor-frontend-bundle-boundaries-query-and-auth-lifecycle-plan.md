...
- [x] 비핵심 workspace management panel이 lazy import 경계로 분리된다.
- [x] `App.tsx`에서 반복되는 read query가 workspace-specific 또는 shared query hook으로 이전된다.
- [x] auth bootstrap/refresh/logout lifecycle이 dedicated auth hook으로 이동한다.
- [ ] long-list 후보 surface(예약/정산)에 대해 적용 여부가 검증되고 결과가 코드 또는 문서로 남는다.
- [x] `members/products` query가 app-shared canonical source로 유지된다는 ownership 규칙이 구현에 반영된다.
- [x] auth state clear 이후 protected UI reset을 누가 호출하는지 경계가 구현에 반영된다.

### Non-Functional Requirements

- [x] 초기 메인 번들에 비활성 workspace 코드가 줄어든다.
- [x] query ownership이 명확해져 `App.tsx`의 fetch orchestration 코드가 감소한다.
- [ ] prototype/jwt 모드에서 auth bootstrapping 회귀가 없다.
- [x] lazy load fallback과 rendering guard 적용 후 UX 점프/깜빡임이 눈에 띄지 않는다.
- [x] `members/products`를 소비하는 cross-workspace surface가 하나의 shared query 결과만 본다.
- [x] logout과 unauthorized 두 경로 모두 동일한 protected reset 계약을 유지한다.

### Quality Gates

- [x] `cd /Users/abc/projects/GymCRM_V2/frontend && npm test` 통과
- [x] `cd /Users/abc/projects/GymCRM_V2/frontend && npm run build` 통과
- [x] `agent-browser` 또는 동등 수동 검증으로 dashboard, memberships, reservations, auth gate 확인
- [x] 최소 1개 이상의 auth hook 테스트 추가
- [x] 최소 1개 이상의 query ownership/invalidation 테스트 추가
- [x] lazy split 후 화면 진입 smoke 증빙 추가
- [x] build 결과에서 split chunk 생성 여부와 main chunk 변화가 기록된다.
- [x] prototype / jwt 두 모드의 auth bootstrap smoke 결과가 분리 기록된다.
- [x] `members/products` shared query를 소비하는 대표 surface(access/lockers/dashboard 또는 members/products management) 중 최소 1개 이상에서 same-source 계약이 검증된다.
- [x] logout / unauthorized 각각에서 protected UI reset이 동일하게 실행되는 검증이 추가된다.
...
- [x] members/products/reservation schedules/settlement/crm 우선순위 확정
- [x] query hook API 정의
- [x] mutation 이후 reload/invalidate 계약 정리
