...
- [ ] 관리자 포털에서 회원 등록/목록/상세/수정이 가능하다 (중복 연락처 검증 포함).
- [ ] 관리자 포털에서 기간제/횟수제 상품 등록/수정/비활성화가 가능하다.
- [ ] 회원 상세 화면에서 회원권 구매가 가능하며, 수기 결제 기록이 함께 생성된다.
- [ ] 프로토타입 API/화면에서 식별자는 숫자 PK로 일관되게 사용된다(표시용 비즈니스 ID 미포함).
- [ ] 회원권 홀딩 신청 시 정책 검증(허용 여부/횟수/기간) 후 홀딩 이력이 저장되고 상태가 변경된다.
- [ ] 회원권 홀딩 해제 시 실제 홀딩 반영 일수 기준으로 만료일이 재산정된다.
- [ ] 회원권 환불 시 프로토타입 고정 단순 정책(비례 사용분 + 10% 위약금)으로 환불액 계산/미리보기/확정이 가능하다.
- [ ] 구매/홀딩/환불 플로우는 중복 제출/중복 처리에 대한 최소 보호가 있다.
- [ ] 프로토타입 범위 외 기능(API/화면)은 명시적으로 미구현 또는 비활성 상태로 처리된다.

### Non-Functional Requirements (Prototype-Adjusted)

- [ ] 프로토타입은 단일 센터 가정으로 동작하되 `center_id` 필드를 유지한다 (see brainstorm: docs/brainstorms/2026-02-23-gym-crm-product-brainstorm.md).
- [ ] 인증 없이 단일 관리자 모드로 동작하지만, 추후 인증 도입을 막지 않도록 API/서비스 경계를 유지한다 (see brainstorm: docs/brainstorms/2026-02-23-gym-crm-product-brainstorm.md).
- [ ] no-auth 모드는 `dev`/`staging`에서만 허용되며 `prod`에서는 실행 차단된다.
- [ ] 주요 테이블에 Soft Delete/Audit 컬럼 규칙을 적용한다.
- [ ] 핵심 업무 플로우(구매/홀딩/환불)는 트랜잭션 경계가 정의되어 부분 저장을 방지한다.

### Quality Gates

- [ ] 데모 시나리오 문서가 존재하고 실제 실행 결과가 확인되었다.
- [ ] 수동 테스트 체크리스트가 작성 및 실행되었다.
- [ ] 최소 자동 테스트(핵심 서비스/API 일부)가 통과한다.
- [ ] 알려진 제한사항 문서가 존재한다.
