...
sequenceDiagram
    ...
    SEC-->>API: 인증 성공
    API-->>C: 200 OK {accessToken, accessTokenExpiresInSeconds, user}
...
| **Payload** | uid, centerId, username, role, primaryRole, roles, jti, typ | uid, centerId, username, role, primaryRole, roles, familyId, jti, typ |
| **알고리즘** | HS256 (HMAC-SHA256) | HS256 (HMAC-SHA256) |
...
| 역할 | 코드 | 설명 | 부여 대상 |
|------|------|------|-----------|
| **SUPER_ADMIN** | `ROLE_SUPER_ADMIN` | 플랫폼 전역 운영/보안 관리자, 다중 센터 운영 권한 | 플랫폼 운영자 |
| **ADMIN** | `ROLE_ADMIN` | 센터 최고 관리자, 센터 내 모든 운영 기능 접근 가능 | 헬스장 원장/대표 |
...
> 관리자 웹 애플리케이션의 RBAC canonical role set은 `ROLE_SUPER_ADMIN`, `ROLE_ADMIN`, `ROLE_MANAGER`, `ROLE_DESK`, `ROLE_TRAINER`다.

#### 기능별 접근 권한 매트릭스

| 기능 | SUPER_ADMIN | ADMIN | MANAGER | TRAINER | DESK |
|------|:-----------:|:-----:|:-------:|:-------:|:-----:|
| 회원 등록/수정 | O | O | O | X | O |
| 회원 삭제 | O | O | X | X | X |
| 회원 조회 | O | O | O | 담당만 | O |
...
@PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN', 'MANAGER', 'DESK')")
public class MemberController {
    ...
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'ADMIN')")  // 삭제는 SUPER_ADMIN/ADMIN만
...
| 버전 | 날짜 | 작성자 | 변경 내용 |
|------|------|--------|-----------|
| v1.1 | 2026-04-17 | Codex | RBAC canonical role set을 `ROLE_SUPER_ADMIN/ADMIN/MANAGER/DESK/TRAINER`로 정렬하고 JWT payload/권한 매트릭스 예시를 현재 구현 계약으로 갱신 |
