...
이번 브레인스토밍은 `/settlements`를 하나의 정산 진입점으로 유지하면서, 1차에서는 운영 상황판과 매출 추이 분석을 완성하고, 2차에서는 트레이너 정산 조회부터 정산서 출력까지 이어지는 흐름을 명확히 정의하는 데 목적이 있다. 이번 재개 세션에서는 특히 정산서 출력 형식, 트레이너 열람 권한, 매출 리포트 다운로드 형식처럼 planning 전에 제품적으로 확정해야 할 정책 결정을 마무리한다.
...
**[2차: 트레이너 정산 탭]**
- R15. `트레이너 정산` 탭은 월별 기준으로 트레이너별 수업 수와 급여 산정 결과를 조회할 수 있어야 한다.
- R16. 운영자는 조회한 정산 결과를 검토한 뒤 정산 확정까지 이어갈 수 있어야 한다.
- R17. 트레이너는 본인 월간 수업 집계와 급여 산정 결과를 조회할 수 있어야 하지만, 다른 트레이너의 데이터에는 접근할 수 없어야 한다.
- R18. 확정된 정산 결과는 PDF 정산서 형태로 출력할 수 있어야 하며, 트레이너는 본인에게 확정된 정산서만 열람할 수 있어야 한다.
- R19. 정산서 출력은 운영자가 실제 지급 또는 내부 보관 업무에 사용할 수 있는 마감 산출물이어야 한다.
- R20. 트레이너 정산 탭은 매출 분석 탭과 분리된 맥락을 유지하되, 같은 `/settlements` 진입점 안에서 자연스럽게 접근 가능해야 한다.
...
- 트레이너 정산 탭은 월별 조회, 확정, PDF 정산서 출력까지 완결된 업무 흐름의 요구사항을 제공한다.
- 트레이너는 본인 데이터만 안전하게 조회하고, 확정된 본인 정산서를 직접 확인할 수 있다.
...
- 트레이너 열람은 본인 범위로 제한: 트레이너는 본인 월간 집계와 본인 확정 정산서까지 열람할 수 있지만, 확정 권한과 타인 데이터 열람 권한은 갖지 않는다.
...
- 2026-04-08 기준 구현 계약은 `GET /api/v1/settlements/sales-report/export`의 XLSX 다운로드, 운영 권한용 `GET /api/v1/settlements/trainer-payroll`/`document`, 트레이너 전용 `GET /api/v1/settlements/trainer-payroll/my-summary` 미니뷰 조회로 정렬되었다.
- 트레이너는 같은 `/settlements` 진입점을 사용하지만, 미니뷰에서 본인 월간 완료 PT 수업 수만 조회하고 급여 계산/정산 확정/PDF 출력에는 접근하지 않는다.
...
- [Affects R17-R18][Technical] 트레이너 본인 조회와 관리자 전체 조회를 같은 endpoint의 권한 분기/필터링으로 처리할지, 별도 조회 계약으로 나눌지 planning 단계에서 결정해야 한다.
- [Affects R18-R19][Needs research] PDF 정산서에 포함할 최소 필드와 레이아웃 수준을 운영 보관용 기준에 맞춰 어디까지 제공할지 planning 단계에서 검토가 필요하다.
