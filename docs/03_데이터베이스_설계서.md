...
| 시스템 | roles | 역할 (SUPER_ADMIN, ADMIN, MANAGER, TRAINER, DESK) |
...
| role_name | VARCHAR(50) | O | - | 역할명 (SUPER_ADMIN, ADMIN, MANAGER, TRAINER, DESK) |
...
| settlement_date | DATE | O | - | 정산 기준일 (해당 월 1일) |
...
- CK: `ck_settlements_date_first_day` (`settlement_date`는 `settlement_year`, `settlement_month`의 1일)

**운영 메모:**
- `settlements`는 월 기준 canonical settlement header다.
- 정산 범위 분리는 header가 아니라 `settlement_details`의 트레이너별 상세 행으로 표현한다.
- 기존 월별 운영 조회/문서 출력 호환을 위해 `trainer_settlements` 스냅샷 테이블이 브리지 역할로 남아 있다.
...
CREATE INDEX idx_settlements_center_month ON settlements(center_id, settlement_year, settlement_month) WHERE is_deleted = FALSE;
...
| idx_settlements_center_month | settlements | center_id, settlement_year, settlement_month | Partial Composite | 월별 canonical settlement 조회 및 중복 생성 방지 보조 |
...
(2, 'ADMIN', '센터 최고 관리자', CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP, 1),
(3, 'MANAGER', '센터 매니저', CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP, 1),
(4, 'TRAINER', '트레이너', CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP, 1),
(5, 'DESK', '일반 직원(데스크)', CURRENT_TIMESTAMP, 1, CURRENT_TIMESTAMP, 1);

SELECT setval('roles_role_id_seq', 5);
```
